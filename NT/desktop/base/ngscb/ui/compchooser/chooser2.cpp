...
static UINT g_cfDsObjectPicker = 0;
static CRITICAL_SECTION g_csClipboardFormat;
...
)
{
    ...
    case WM_COMMAND:
            ...
            case IDC_CHOOSER2_BUTTON_BROWSE_MACHINENAMES:
                {
                    static wchar_t* sbstrTargetComputer = NULL;
...
bool CALLBACK CHOOSER2_PickTargetComputer(
...
BOOL CALLBACK WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
    if (fdwReason == DLL_PROCESS_ATTACH) {
        g_hInstance = hinstDLL;
        InitializeCriticalSection(&g_csClipboardFormat);
        g_cfDsObjectPicker = RegisterClipboardFormat(CFSTR_DSOP_DS_SELECTION_LIST);
        g_bInitialized = TRUE;
    }
    else if (fdwReason == DLL_PROCESS_DETACH) {
        DeleteCriticalSection(&g_csClipboardFormat);
    }
    return TRUE;
}
