...
BOOL FillInFields(HWND hwnd, PCCERT_CONTEXT pccert)
{
    ...
    LoadString(HinstDll, IDS_SELECT_INFO, (LPSTR)rgwch, ARRAYSIZE(rgwch));
    FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER |
                  FORMAT_MESSAGE_FROM_STRING |
                  FORMAT_MESSAGE_ARGUMENT_ARRAY, rgwch, 0, 0,
                  (LPSTR) &pwsz, 0, (va_list *) rgpwsz);
...
#endif // !WIN16
    SetDlgItemText(hwnd, IDC_CS_INFO, (LPCSTR)pwsz);
    free(rgpwsz[0]);
    free(rgpwsz[1]);
...
BOOL CallCryptUISelect(BOOL fWide, PCERT_SELECT_STRUCT_W pcssW)
{
    ...
    if (fWide) {
        pccert = CryptUIDlgSelectCertificateW((PCCRYPTUI_SELECTCERTIFICATE_STRUCTW)&cscs);
    }
    else {
        pccert = CryptUIDlgSelectCertificateA((PCCRYPTUI_SELECTCERTIFICATE_STRUCTA) &cscs);
    }
...
{
    if (CryptUIAvailable()) {
        return CallCryptUISelect(FALSE, (PCERT_SELECT_STRUCT_W) pCertSelectInfo);
    }
    ...
    if (pCertSelectInfo->cCertStore == 0) {
        ...
        pCertSelectInfo->arrayCertStore = &hCertStore;
    }
    //
    //  Size of the object must be acceptable in order to copy it over
    //

    if (pCertSelectInfo->dwSize > sizeof(cssW)) {
    ...
    if (FIsWin95) {
        ...
        if (pCertSelectInfo->dwFlags & CSS_ENABLETEMPLATEHANDLE) {
            ret = (int) DialogBoxIndirectParamA(pCertSelectInfo->hInstance,
                                  (LPCDLGTEMPLATE_X) pCertSelectInfo->pTemplateName,
                                  pCertSelectInfo->hwndParent, SelectCertDlgProc,
                                  (LPARAM) pCertSelectInfo);
        }
        else if (pCertSelectInfo->dwFlags & CSS_ENABLETEMPLATE) {
            ret = (int) DialogBoxParamA(pCertSelectInfo->hInstance, pCertSelectInfo->pTemplateName,
                                  pCertSelectInfo->hwndParent, SelectCertDlgProc,
                                  (LPARAM) pCertSelectInfo);
        }
        else {
        ...
        memcpy(&cssW, pCertSelectInfo, (int)min(pCertSelectInfo->dwSize, sizeof(cssW)));

        if (pCertSelectInfo->szTitle != NULL) {
            cch = MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, pCertSelectInfo->szTitle, -1,
                                      NULL, 0);
            ...
            MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, pCertSelectInfo->szTitle, -1,
                                (LPWSTR) cssW.szTitle, cch+1);
        }

        if (pCertSelectInfo->szHelpFileName != NULL) {
            cch = MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED,
                                      pCertSelectInfo->szHelpFileName, -1, NULL, 0);
            cssW.szHelpFileName = (LPWSTR) malloc((cch+1)*sizeof(WCHAR));
            if (cssW.szHelpFileName == NULL) {
                SetLastError(ERROR_NOT_ENOUGH_MEMORY);
                goto ExitW;
            }
            MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, pCertSelectInfo->szHelpFileName, -1,
                                (LPWSTR) cssW.szHelpFileName, cch+1);
        }

        if (pCertSelectInfo->dwFlags & CSS_ENABLETEMPLATE) {
            cch = MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED,
                                      pCertSelectInfo->pTemplateName, -1, NULL, 0);
            cssW.pTemplateName = (LPWSTR) malloc((cch+1)*sizeof(WCHAR));
            if (cssW.pTemplateName == NULL) {
                SetLastError(ERROR_NOT_ENOUGH_MEMORY);
                goto ExitW;
            }
            MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, pCertSelectInfo->pTemplateName, -1,
        ...
        pCertSelectInfo->cCertContext = cssW.cCertContext;
...
Exit:
    if (hCertStore != NULL) {
        ...
        pCertSelectInfo->cCertStore = 0;
        pCertSelectInfo->arrayCertStore = NULL;
    }
    return (ret == IDOK);
}
...
{
    if (CryptUIAvailable()) {
        return CallCryptUISelect(TRUE, pCertSelectInfo);
    }
    ...
    if (pCertSelectInfo->dwFlags & CSS_ENABLETEMPLATEHANDLE) {
        ret = (int) DialogBoxIndirectParam(pCertSelectInfo->hInstance,
                                     (LPCDLGTEMPLATE_X) pCertSelectInfo->pTemplateName,
                                     pCertSelectInfo->hwndParent, SelectCertDlgProc,
                                     (LPARAM) pCertSelectInfo);
    }
    else if (pCertSelectInfo->dwFlags & CSS_ENABLETEMPLATE) {
        ret = (int) DialogBoxParam(pCertSelectInfo->hInstance, (LPCSTR)pCertSelectInfo->pTemplateName,
                             pCertSelectInfo->hwndParent, SelectCertDlgProc,
                             (LPARAM) pCertSelectInfo);
    }
    else {
