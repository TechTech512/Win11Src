        ...
        PCCERT_CONTEXT pccertSigner)    // Certificate to be used in signing trust list
{
    ...
    for (i = 0; i < cCerts; i++) {
        PCCERT_CONTEXT pCert = rgCerts[i].pccert;
        DWORD dwError = S_OK;

        switch(rgCerts[i].dwOperation) {
...
LPWSTR FormatValidityFailures(DWORD dwFlags)
{
    ...
    if (dwFlags & CERT_VALIDITY_BEFORE_START) {
        LoadString(g_hCertTrustInst, IDS_WHY_NOT_YET, (LPSTR)rgwch, sizeof(rgwch)/sizeof(WCHAR));
    ...
    if (dwFlags & CERT_VALIDITY_AFTER_END) {
        LoadString(g_hCertTrustInst, IDS_WHY_EXPIRED, (LPSTR)rgwch, sizeof(rgwch)/sizeof(WCHAR));
    ...
    if (dwFlags & CERT_VALIDITY_SIGNATURE_FAILS) {
        LoadString(g_hCertTrustInst, IDS_WHY_CERT_SIG, (LPSTR)rgwch, sizeof(rgwch)/sizeof(WCHAR));
        if (wcslen(pwsz) + wcslen(rgwch) + 2 > cch) {
            cch += 200;
            pwszT = (LPWSTR) realloc(pwsz, cch*sizeof(WCHAR));
            if (pwszT == NULL) {
    ...
    if (dwFlags & CERT_VALIDITY_NO_ISSUER_CERT_FOUND) {
        LoadString(g_hCertTrustInst, IDS_WHY_NO_PARENT, (LPSTR)rgwch, sizeof(rgwch)/sizeof(WCHAR));
        if (wcslen(pwsz) + wcslen(rgwch) + 2 > cch) {
            cch += 200;
            pwszT = (LPWSTR) realloc(pwsz, cch*sizeof(WCHAR));
            if (pwszT == NULL) {
    ...
    if (dwFlags & CERT_VALIDITY_NO_CRL_FOUND) {
        LoadString(g_hCertTrustInst, IDS_WHY_NO_CRL, (LPSTR)rgwch, sizeof(rgwch)/sizeof(WCHAR));
        if (wcslen(pwsz) + wcslen(rgwch) + 2 > cch) {
            cch += 200;
            pwszT = (LPWSTR) realloc(pwsz, cch*sizeof(WCHAR));
            if (pwszT == NULL) {
    ...
    if (dwFlags & CERT_VALIDITY_CERTIFICATE_REVOKED) {
        LoadString(g_hCertTrustInst, IDS_WHY_REVOKED, (LPSTR)rgwch, sizeof(rgwch)/sizeof(WCHAR));
        if (wcslen(pwsz) + wcslen(rgwch) + 2 > cch) {
            cch += 200;
            pwszT = (LPWSTR) realloc(pwsz, cch*sizeof(WCHAR));
            if (pwszT == NULL) {
    ...
    if (dwFlags & CERT_VALIDITY_CRL_OUT_OF_DATE) {
        LoadString(g_hCertTrustInst, IDS_WHY_CRL_EXPIRED, (LPSTR)rgwch, sizeof(rgwch)/sizeof(WCHAR));
        if (wcslen(pwsz) + wcslen(rgwch) + 2 > cch) {
            cch += 200;
            pwszT = (LPWSTR) realloc(pwsz, cch*sizeof(WCHAR));
            if (pwszT == NULL) {
    ...
    if (dwFlags & CERT_VALIDITY_EXTENDED_USAGE_FAILURE) {
        LoadString(g_hCertTrustInst, IDS_WHY_EXTEND_USE, (LPSTR)rgwch, sizeof(rgwch)/sizeof(WCHAR));
        if (wcslen(pwsz) + wcslen(rgwch) + 2 > cch) {
            cch += 200;
            pwszT = (LPWSTR) realloc(pwsz, cch*sizeof(WCHAR));
            if (pwszT == NULL) {
    ...
    if (dwFlags & CERT_VALIDITY_NAME_CONSTRAINTS_FAILURE) {
        LoadString(g_hCertTrustInst, IDS_WHY_NAME_CONST, (LPSTR)rgwch, sizeof(rgwch)/sizeof(WCHAR));
        if (wcslen(pwsz) + wcslen(rgwch) + 2 > cch) {
            cch += 200;
            pwszT = (LPWSTR) realloc(pwsz, cch*sizeof(WCHAR));
            if (pwszT == NULL) {
    ...
    if (dwFlags & CERT_VALIDITY_POLICY_FAILURE) {
        LoadString(g_hCertTrustInst, IDS_WHY_POLICY, (LPSTR)rgwch, sizeof(rgwch)/sizeof(WCHAR));
        if (wcslen(pwsz) + wcslen(rgwch) + 2 > cch) {
            cch += 200;
            pwszT = (LPWSTR) realloc(pwsz, cch*sizeof(WCHAR));
            if (pwszT == NULL) {
    ...
    if (dwFlags & CERT_VALIDITY_BASIC_CONSTRAINTS_FAILURE) {
        LoadString(g_hCertTrustInst, IDS_WHY_BASIC_CONS, (LPSTR)rgwch, sizeof(rgwch)/sizeof(WCHAR));
        if (wcslen(pwsz) + wcslen(rgwch) + 2 > cch) {
            cch += 200;
            pwszT = (LPWSTR) realloc(pwsz, cch*sizeof(WCHAR));
            if (pwszT == NULL) {
