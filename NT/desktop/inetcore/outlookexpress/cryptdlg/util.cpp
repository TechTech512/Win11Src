...
BOOL LoadStringInWindow(HWND hwnd, UINT idCtrl, HMODULE hmod, UINT idString)
{
    ...
    if (FIsWin95) {
        ...
        SetDlgItemText(hwnd, idCtrl, (LPCSTR)rgwch);
    }
#endif  // !MAC
#endif  // !WIN16
...
BOOL FormatAlgorithm(HWND hwnd, UINT id, PCCERT_CONTEXT pccert)
{
    ...
    if (strcmp(psz, szOID_RSA_RSA) == 0) {
        ...
        SetDlgItemText(hwnd, id, (LPCSTR)pwsz);
        free(pwsz);
        return TRUE;
    }
    ...
    FormatMessage(FORMAT_MESSAGE_FROM_STRING | FORMAT_MESSAGE_ARGUMENT_ARRAY |
                  ...
                  L"%1 (%2 bits)", 0, (DWORD)0, (LPSTR) &pszMsg, 0, (va_list *) rgdw);

    SetDlgItemText(hwnd, id, (LPCSTR)pszMsg);
#ifndef WIN16
...
BOOL FormatBinary(HWND hwnd, UINT id, LPBYTE pb, DWORD cb)
{
    ...
    SetDlgItemText(hwnd, id, (LPCSTR)pwch);
    free(pwch);
    return TRUE;
}
...
BOOL FormatDate(HWND hwnd, UINT id, FILETIME ft)
{
    ...
    cch2 = GetDateFormat(LOCALE_USER_DEFAULT, 0, &st, NULL, (LPSTR)pwsz, cch);
    cch2 -= 1;
    pwsz[cch2++] = ' ';

    GetTimeFormat(LOCALE_USER_DEFAULT, 0, &st, NULL, (LPSTR)&pwsz[cch2], cch-cch2);
...
BOOL FormatIssuer(HWND hwnd, UINT id, PCCERT_CONTEXT pccert, DWORD dwFlags)
{
    ...
    CertNameToStrW(CRYPT_ASN_ENCODING, &pccert->pCertInfo->Issuer,
                   dwFlags | CERT_NAME_STR_CRLF_FLAG,
                   psz, cch);
    SetDlgItemText(hwnd, id, (LPCSTR)psz);
    free(psz);
    return TRUE;
}
...
BOOL FormatSerialNo(HWND hwnd, UINT id, PCCERT_CONTEXT pccert)
{
    ...
    SetDlgItemText(hwnd, id, (LPCSTR)rgwch);
    return TRUE;
}
...
BOOL FormatValidity(HWND hwnd, UINT id, PCCERT_CONTEXT pccert)
{
    ...
    GetDateFormat(LOCALE_USER_DEFAULT, DATE_SHORTDATE, &stNotBefore,
                  NULL, (LPSTR)rgwchNotBefore, sizeof(rgwchNotBefore)/sizeof(WCHAR));
    GetDateFormat(LOCALE_USER_DEFAULT, DATE_SHORTDATE, &stNotAfter,
                  NULL, (LPSTR)rgwchNotAfter, sizeof(rgwchNotAfter)/sizeof(WCHAR));
    LoadString(HinstDll, IDS_VALIDITY_FORMAT, (LPSTR)rgwchFormat,
    ...
    FormatMessage(FORMAT_MESSAGE_FROM_STRING | FORMAT_MESSAGE_ARGUMENT_ARRAY,
                  rgwchFormat, 0,  0,
                  (LPSTR)rgwchValidity, sizeof(rgwchValidity)/sizeof(WCHAR),
                  (va_list *) rgdw);

    SetDlgItemText(hwnd, id, (LPCSTR)rgwchValidity);
#endif  // !MAC
    return TRUE;
}
...
LPWSTR PrettySubject(PCCERT_CONTEXT pccert)
{
    ...
    pwsz = (LPWSTR) malloc(cch*sizeof(TCHAR));
    if (pwsz != NULL) {
        CertNameToStr(CRYPT_ASN_ENCODING, &pccert->pCertInfo->Subject,
                            CERT_SIMPLE_NAME_STR, (LPSTR)pwsz, cch);
    }
    ...
    DWORD /*cbEncoded*/, void * pbFormat, DWORD * pcbFormat)
{
    ...
    cch = LoadString(HinstDll, IDS_EMAIL_DESC, (LPSTR)rgwch, sizeof(rgwch)/sizeof(WCHAR));
