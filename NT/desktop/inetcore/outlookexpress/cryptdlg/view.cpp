...
INT_PTR CALLBACK ViewPageGeneral(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        LoadString(HinstDll, IDS_GENERAL_INFO, (LPSTR)rgwch, sizeof(rgwch)/sizeof(WCHAR));
        FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER |
                      FORMAT_MESSAGE_FROM_STRING |
                      FORMAT_MESSAGE_ARGUMENT_ARRAY, rgwch,
                      0, 0, (LPSTR) &pwsz, 0, (va_list *) rgpwsz);

        SetDlgItemText(hwndDlg, IDC_TEXT, (LPCSTR)pwsz);
...
INT_PTR CALLBACK ViewPageDetails(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        SetDlgItemText(hwndDlg, IDC_FRIENDLY_NAME, (LPCSTR)pwsz);
        free(pwsz);
        ...
        if (fInvalid) {
            ...
            GetWindowRect(GetDlgItem(hwndDlg, IDC_TRUST_GROUP), &ti.rect);
            ti.uId = 0;
            ti.lpszText = (LPSTR)FormatValidityFailures(pviewhelp->rgpcf[0]->m_dwFlags);
    ...
    case WM_NOTIFY:
        ...
        case PSN_APPLY:
            ...
            if (cch) {
                ...
                if (pwsz) {
                    GetDlgItemText(hwndDlg, IDC_FRIENDLY_NAME, (LPSTR)pwsz, cch+1);
    ...
    case WM_INITDIALOG:
        ...
        if (pviewhelp->pcvp->cArrayPurposes == 1) {
            ...
            f = CryptFormatObject(X509_ASN_ENCODING, 0, 0, NULL,
                                  ...
                                  NULL, 0, rgwch, &cb);
            if (f && (rgwch[0] != 0)) {
                SetDlgItemText(hwndDlg, IDC_TRUST_EDIT, (LPCSTR)rgwch);
            }
            else {
        ...
        for (i=pviewhelp->ccf-1; i>= 0; i--) {
            ...
            tvins.item.pszText = (LPSTR)pwsz;
            tvins.item.cchTextMax = lstrlen((LPCSTR)pwsz);
                               ...
                               LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        for (i=IDS_ADV_VERSION; i<= IDS_ADV_PUBKEY; i++) {
            LoadString(HinstDll, i, (LPSTR)rgwch, sizeof(rgwch)/sizeof(WCHAR));
            ...
            if (HIWORD(wParam) == LBN_SELCHANGE) {
                ...
                if (i <= IDS_ADV_PUBKEY - IDS_ADV_VERSION) {
                    ...
                    case IDS_ADV_VERSION:         // Version
                        ...
                        rgwch[2] = 0;
                        SetDlgItemText(hwndDlg, IDC_EDIT1, (LPCSTR)rgwch);
                        break;
                    ...
                    if (f && (cb > 0)) {
                        ...
                        CryptFormatObject(X509_ASN_ENCODING, 0, 0, NULL,
                                    ...
                                    pccert->pCertInfo->rgExtension[i].Value.cbData,
                                    pwsz, &cb);
                        SetDlgItemText(hwndDlg, IDC_EDIT1, (LPCSTR)pwsz);
...
BOOL CertViewUI(BOOL fWide, PCERT_VIEWPROPERTIES_STRUCT_W pcvp)
{
    ...
    if ((NULL != ppsp) && 
        ...
        if (!(pcvp->dwFlags & CM_HIDE_TRUSTPAGE)) {
            ...
            ppsp[iPage].pszTemplate = (LPCWSTR)MAKEINTRESOURCE(IDD_CRYPTUI_CERTPROP_TRUST);
            ppsp[iPage].hIcon = 0;
...
BOOL CertViewPropertiesX(PCERT_VIEWPROPERTIES_STRUCT_W pcvp)
{
    ...
    if (FIsWin95) {
        ...
        hdr.nPages = cPages;
        hdr.nStartPage = pcvp->nStartPage;
        hdr.ppsp = (LPCPROPSHEETPAGEW)ppage;
        hdr.pfnCallback = NULL;
