    ...
    )
{
    ...
    StringCbPrintf(
        buf,
        (size_t)"devID=%d, devClass='%ws'",
        (STRSAFE_LPCSTR)dwDeviceID,
    ...
    )
{
    ...
    StringCbPrintf (buf, (size_t)"providerID=%d", (STRSAFE_LPCSTR)dwPermanentProviderID);
    ...
    )
{
    ...
    StringCbPrintf(
        buf,
        (size_t)"htDlgInst=x%x, lpParams='%p', dwSize=x%x",
        (STRSAFE_LPCSTR)htDlgInst,
        lpParams,
        dwSize
        );
