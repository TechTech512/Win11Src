/*++  BUILD Version: 0001   // Increment this if a change has global effects.
   ...
   Abstract:
      ...
      Generic SNMP Extension Agent helper routines for walking a MIB
      table and resolving variable bindings.  The specific MIB (FTP
      counters, in this build) is supplied by the caller via the
      MIB_ENTRIES structure; this module contains no service-specific
      OIDs or tables.

   Author:

       Murali R. Krishnan    ( MuraliK )     23-Feb-1995
       Emily  B. Kruglick    ( EmilyK)       02-Nov-2000
   ...
   Project:
       ...
       SNMP Extension DLL for FTP Service DLL
...
/*
 * SNMP OID helper wrappers.  These map the symbolic names used
 * by the original source onto the Win32 SNMP utility API.
 */
#ifndef SNMP_oidcpy
...
--*/
{
    ...
    if ( pduResult == SNMP_ERRORSTATUS_NOERROR) {
        ...
        //
        pduResult = MIB_leaf_func( pRfcVarBinding, pduAction, pMibeCurrent,
    ...
    switch( pduAction ) {
      ...
      case MIB_GETNEXT:
        ...
        if ( ( pMibeCurrent >= pMibEntries->prgMibEntry) &&
            ...
            pduResult = GetNextVar( pRfcVarBinding,
                                    pMibeCurrent,
                                    pMibEntries,
                                    pStatistics);
        }
