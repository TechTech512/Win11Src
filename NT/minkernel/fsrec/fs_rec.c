...

{
    ...
    (NTSTATUS (__stdcall *)(PDEVICE_OBJECT,PIRP,UCHAR))DriverObject->MajorFunction[IRP_MJ_FILE_SYSTEM_CONTROL] = FsRecFsControl;
...

{
    ...
    switch ( deviceExtension->FileSystemType ) {
        ...
        case ReFSFileSystem:
            ...
            status = ReFSRecFsControl( DeviceObject, Irp, Flag );
            break;

        case ReFSv1FileSystem:

            status = ReFSRecFsControl( DeviceObject, Irp, Flag );
            break;

        case btrfsFileSystem:

            status = BtrfsRecFsControl( DeviceObject, Irp );
            break;

        case ffsFileSystem:

            status = FfsRecFsControl( DeviceObject, Irp );
            break;

        case reiserfsFileSystem:

            status = ReiserfsRecFsControl( DeviceObject, Irp );
            break;

        case Ext2fsFileSystem:

            status = ExtRecFsControl( DeviceObject, Irp );
            break;

        case vfatfsFileSystem:

            status = FatxRecFsControl( DeviceObject, Irp );
            break;
