...
VOID FitTextToButton ( HWND hDlg, INT nButtonID, LPTSTR szName )
{
    ...
    if (szName == NULL || hDlg == NULL) return;
    ...
    // Check if text fits
    if (nLen > 0 && GetTextExtentPoint32(hDC, szName, nLen, &size))
    {
        pt.x = size.cx;
        LPtoDP(hDC, &pt, 1);
        
        // If text doesn't fit, truncate it
        if (pt.x > rect.right && nLen > 3)
        {
            int i;
            ...
            for (i = nLen - 1; i > 3; i--)
            {
                ...
                if (GetTextExtentPoint32(hDC, szName, i, &size))
                {
                    pt.x = size.cx;
                    LPtoDP(hDC, &pt, 1);
                    
                    if (pt.x <= rect.right - 30) // Allow space for "..."
                    {
                        szName[i] = chTemp;
                        break;
                    }
...
BOOL Is911 ( LPLINETRANSLATEOUTPUT lpTransOut )
{
    ...
    if (lpTransOut->dwDialableStringOffset == 0 || 
        lpTransOut->dwDialableStringSize == 0 ||
        lpTransOut->dwDialableStringSize > 512)
    {
        return FALSE;
    }
    
    // Get pointer to string data
    pData = (BYTE*)lpTransOut;
    pString = pData + lpTransOut->dwDialableStringOffset;
    
    // Copy to local buffer safely
    dwCopySize = lpTransOut->dwDialableStringSize;
    if (dwCopySize > sizeof(szBuffer) - 1)
        dwCopySize = sizeof(szBuffer) - 1;
    
    memcpy(szBuffer, pString, dwCopySize);
    szBuffer[dwCopySize] = '\0';
    
    // Scan for first 3 digits
    for (i = 0; i < dwCopySize && j < 3; i++)
    {
        if (szBuffer[i] >= '0' && szBuffer[i] <= '9')
        {
            szDigits[j++] = szBuffer[i];
        }
    }
    
    // Check for "911"
    if (j == 3 && szDigits[0] == '9' && szDigits[1] == '1' && szDigits[2] == '1')
        return TRUE;
...
BOOL MakeCanonicalNumber ( LPCTSTR szNumber, LPTSTR szCanNumber )
{
    ...
    // Clear all buffers using memset with compile-time constants
    memset(szBuffer, 0, sizeof(szBuffer));
    memset(szDigits, 0, sizeof(szDigits));
    memset(szAreaCode, 0, sizeof(szAreaCode));

    if (szNumber == NULL || szCanNumber == NULL)
        return FALSE;
    ...
    do
    {
        ...
        while (TRUE)
        {
            ...
            errCode = lineTranslateAddress(ghLineApp, giCurrentLine,
                        gCurrentLineInfo.dwAPIVersion, szNumber, 0, 0, lpTransOut);
            if ((LINEERR_INIFILECORRUPT != errCode) || (FALSE == bTryAgain))
                break;

            bTryAgain = FALSE;
            errCode = lineTranslateDialog(ghLineApp, giCurrentLine,
                        TAPI_VERSION_1_4, NULL, szNumber);
            if (NO_ERROR != errCode)
                break;
        }
    ...
    // Copy szNumber to char buffer using pointer arithmetic
    pSrc = (char*)szNumber;
    pDst = szBuffer;
    nLen = 0;
    while (*pSrc != '\0' && nLen < 500)
    {
        *pDst = (char)(*pSrc & 0x7F);
        pSrc++;
        pDst++;
    ...
    *pDst = '\0';
    ...
    // Find the '|' character using pointer scanning
    pTemp = szBuffer;
    while (*pTemp != '\0' && *pTemp != '|')
    {
        pTemp++;
    }
    nLen = (int)(pTemp - szBuffer);

    // Validate characters are only digits and allowed punctuation
    pTemp = szBuffer;
    for (dwInd = 0; dwInd < (DWORD)nLen; dwInd++)
    {
        ch = *pTemp;
        if (!((ch >= '0' && ch <= '9') ||
              ch == ' ' || ch == '(' || ch == ')' ||
    ...
    // Extract digits only using pointer arithmetic
    pTemp = szBuffer;
    pDst = szDigits;
    nDigitCount = 0;
    for (dwInd = 0; dwInd < (DWORD)nLen; dwInd++)
    ...
    *pDst = '\0';
    ...
    if (nDigitCount < LOCAL_NUMBER)
        goto error;

    // Check for extended local number (8 digits starting with 0/1)
    if (nDigitCount == EXTENDED_LOCAL_NUMBER)
    {
        pTemp = szDigits;
        if (*pTemp == '0' || *pTemp == '1')
        {
            // Shift left by moving pointer
            pTemp++;
            pDst = szDigits;
            while (*pTemp != '\0')
            {
    ...
    if (nDigitCount == LOCAL_NUMBER)
    {
        ...
        do
        {
            ...
            errCode = lineGetTranslateCaps(ghLineApp,
                        gCurrentLineInfo.dwAPIVersion, lpTransCaps);
            if (errCode)
            {
                errString(ghWndMain, errCode, MB_ICONSTOP | MB_OK);
                goto error;
        ...
        lpLocLst = (LPLINELOCATIONENTRY)((char*)lpTransCaps +
        ...
        // Build canonical number: +1 (area code) number
        szCanNumber[0] = TEXT('\0');
        lstrcat(szCanNumber, TEXT("+1 ("));

        lpCityCode = (LPTSTR)((char*)lpTransCaps + lpLocLst[dwInd].dwCityCodeOffset);
        lstrcat(szCanNumber, lpCityCode);
        lstrcat(szCanNumber, TEXT(") "));

        // Append the 7 digits using pointer walking
        pTemp = szDigits;
        for (dwInd = 0; dwInd < (DWORD)nDigitCount; dwInd++)
        {
            TCHAR digit[2];
            digit[0] = (TCHAR)*pTemp;
            digit[1] = TEXT('\0');
            lstrcat(szCanNumber, digit);
            pTemp++;
        }

        // Add suffix after '|' if present
        pTemp = szBuffer;
        while (*pTemp != '\0' && *pTemp != '|')
        {
            pTemp++;
        ...
        if (*pTemp == '|')
        {
            ...
            *pDst = '\0';
            lstrcat(szCanNumber, suffix);
        }

        bRes = TRUE;
    }
    // Handle extended long distance (11 digits starting with 0/1)
    else if (nDigitCount == EXTENDED_LONG_DISTANCE_NUMBER)
    {
        pTemp = szDigits;
        if (*pTemp == '0' || *pTemp == '1')
        {
    ...
    // Handle long distance number (10 digits)
    if (nDigitCount == LONG_DISTANCE_NUMBER)
    {
        pTemp = szDigits;
        // Check first digit
        if (*pTemp == '0' || *pTemp == '1')
            goto error;
        // Check fourth digit (skip first 3)
        pTemp = szDigits + 3;
        if (*pTemp == '0' || *pTemp == '1')
            goto error;
