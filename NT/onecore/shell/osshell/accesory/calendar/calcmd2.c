...
INT CheckMarginNums(HWND hWnd)
    ...
    xPrintRes = (WORD)GetDeviceCaps(hPrintDC, HORZRES);
    yPrintRes = (WORD)GetDeviceCaps(hPrintDC, VERTRES);
    xPixInch  = (WORD)GetDeviceCaps(hPrintDC, LOGPIXELSX);
    yPixInch  = (WORD)GetDeviceCaps(hPrintDC, LOGPIXELSY);

    DeleteDC(hPrintDC);

    /* margin values have int/float values. Do range check */
    GetDlgItemText(hWnd, IDCN_EDITMARGINLEFT, szStr, PT_LEN);
    Left     = (WORD)atopix(szStr,xPixInch);

    GetDlgItemText(hWnd, IDCN_EDITMARGINRIGHT, szStr, PT_LEN);
    Right    = (WORD)atopix(szStr, xPixInch);

    GetDlgItemText(hWnd, IDCN_EDITMARGINTOP, szStr, PT_LEN);
    Top      = (WORD)atopix(szStr, yPixInch);

    GetDlgItemText(hWnd, IDCN_EDITMARGINBOT, szStr, PT_LEN);
    Bottom   = (WORD)atopix(szStr, yPixInch);
...
BOOL APIENTRY FnControls (
     ...
     switch (message)
	  ...
	  case WM_COMMAND:
		switch (GET_WM_COMMAND_ID(wParam, lParam)) {
		    case IDOK:
			 cMinEarlyRing = (WORD)GetDlgItemInt (hwnd, IDCN_EARLYRING,
			      &fOk, FALSE);
