...
VOID FAR APIENTRY Print ()
     ...
     */
     ...
     CharPrintHeight = (SHORT)Metrics.tmHeight + (SHORT)Metrics.tmExternalLeading;
     CharPrintWidth = (SHORT)(Metrics.tmAveCharWidth + Metrics.tmMaxCharWidth)/2;             /* character width */
     ...
     dyTop      = (SHORT)atopix(chPageText[4], yPixInch);
     dyBottom   = (SHORT)atopix(chPageText[5], yPixInch);
     dxLeft     = (SHORT)atopix(chPageText[2], xPixInch);
     dxRight    = (SHORT)atopix(chPageText[3], xPixInch);
        ...
        if (*src == '&')   /* is it the escape char? */
            ...
            if (*src == letters[0] || *src == letters[1])
                {                      /* &f file name (no path) */
                    ...
                    nIndex[nAlign] += (SHORT)lstrlen((LPSTR)ptr);
                }
            else
            ...
            if (*src == letters[2] || *src == letters[3])
                {                      /* &P or &P+num page */
                    ...
                    nIndex[nAlign] += (SHORT)lstrlen((LPSTR)buf);
                    src--;
                }
            else
