#include <windows.h>
#include <winternl.h>  // Add this at the top with other includes
#include <setupapi.h>
    ...
    if (!CheckedEms || i != 0 ) {
        ...
        if( h ){
            ...
            if (NewHardware) {
...
#if DBG
                // Debug-only PEB access (if absolutely needed)
                typedef struct _PEB {
                    BYTE Reserved1[2];
                    BYTE BeingDebugged;
                    BYTE Reserved2[1];
                ...
                PPEB Peb = (PPEB)__readfsdword(0x30); // For x86
                // For x64: (PPEB)__readgsqword(0x60)
    ...
    )
{
    ...
    FP_PROMPTREBOOT           fpPromptReboot = NULL;
