...
HRESULT HrDisableSystemSoftCardWizard()
{
   ...
   if (CallUninstallFunction(TEXT("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\CardWizard for Windows NT"),
	 ...
	 if (!fStop) {
	    ...
	    TCHAR* pszStart = NULL;
	    TCHAR* pszEnd = NULL;
	    pszEnd = (TCHAR*)_tcsstr((wchar_t*)szVal, TEXT("WizNT.exe -L"));
	    if (pszEnd) {
	       pszEnd = pszEnd + 12;
	    } else {
	    ...
	    if (!fStop) {
		  ...
		  pszStart = pszTemp;
		  pszTemp = (TCHAR*)_tcsstr((const wchar_t*)pszStart + 1, TEXT(","));
		  if (!pszTemp)
		     break;
	       }
	       if (pszStart == NULL) {
		...
		int i = 0;  // Declare i at the beginning of the scope
	    if (!fStop) {
	       int sslen = (int)(pszEnd - pszStart);
	       szSubString = new TCHAR[sslen + 1];
	       for (i = 0; i < sslen; i++) {  // Now i is accessible
