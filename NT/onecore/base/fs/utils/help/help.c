...
// Data section
static unsigned long UnusedMessages[2] = { HELP_PAGEFILECONFIG_HELP_MESSAGE, HELP_EVENTQUERY_HELP_MESSAGE };
...
--*/
{
    ...
    for (Message = HELP_FIRST_HELP_MESSAGE; RetValue && Message <= HELP_LAST_HELP_MESSAGE; Message++) {
        ...
        offset = 0;
        do {
            if (Message == UnusedMessages[offset / 4]) {
                goto skip_message;
            }
            offset = offset + 4;
        } while (offset < sizeof(UnusedMessages));
...
--*/
{
    ...
    if (MessageString == NULL) {
        DisplayMessageError( ERROR_MR_MID_NOT_FOUND );
        return FALSE;
    }
