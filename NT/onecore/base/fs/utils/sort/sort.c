...
TCHAR *get_string(int id)
{
    ...
    static char fallbackBuffer[MESSAGE_BUFFER_LENGTH];

    // Try to load from the message resource (if present)
    HINSTANCE hInst = GetModuleHandle(NULL);
    bytes = FormatMessageA(FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_IGNORE_INSERTS,
                           hInst,
                           id,
                           0,
                           messageBuffer,
    ...
    if (bytes > 0) {
        // Convert from ANSI to OEM (as original did)
        w_str = HeapAlloc(GetProcessHeap(), 0, (bytes + 1) * sizeof(wchar_t));
        if (w_str) {
            status = RtlMultiByteToUnicodeN(w_str,
                                            bytes * sizeof(wchar_t),
	...
	strcpy(messageBuffer, id);
	return "\r\n";
}


/* USAGE - print the /? usage message to the standard output.
 * Uses the message table (if present) with a hardcoded fallback.
 */
void usage()
{
    ...
    if (FormatMessageA(FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_IGNORE_INSERTS,
                       hInst, MSG_SORT_USAGE1, 0, buf, sizeof(buf), NULL) == 0)
    {
        get_string(MSG_SORT_USAGE1);
    }
    WriteFile(hOut, buf, strlen(buf), &written, NULL);

    // Second part (ID 7415)
    if (FormatMessageA(FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_IGNORE_INSERTS,
                       hInst, MSG_SORT_USAGE2, 0, buf, sizeof(buf), NULL) == 0)
    {
...
void warning(int id)
{
    ...
    DWORD written;
    WriteFile(hErr, msg, strlen(msg), &written, NULL);
	WriteFile(hErr, "\r\n", 2, &written, NULL);
...
int get_sector_size(TCHAR *path)
{
    ...
    if ( (path != NULL) && (strlen(path) < (sizeof(buf)/sizeof(buf[0])) ) ) {
        strcpy_s((char *)buf, (rsize_t)NULL, (const char *)path);
    }
...
void init_two_pass()
{
    ...
    if (Temp_dir != NULL)
        strcpy_s((char*)temp_path, (rsize_t)NULL, (const char *)Temp_dir);
    else
 ...
 *
 * NOTE: This version inlines the scheduling code (the same logic as
 * sched_run_read) to match the decompiled binary exactly.
 */
void queue_run_read(run_t *run)
{
    ...
    /* if we can schedule a read immediately, do so (inline). */
    if (Reads_completed + MAX_IO <= Reads_issued)
        return;
    ...
    {
        transfer = (int)(run->end_off - buf_off);
        int rem = transfer & (Temp_sector_size - 1);
        if (rem)
            transfer += Temp_sector_size - rem;
    }
