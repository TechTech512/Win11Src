...
long ApiSetExists_ext_ms_win_shell_shlwapi_l1_1_0 = 0;
UNICODE_STRING ApiSetName_ext_ms_win_shell_shlwapi_l1_1_0 = {0x3E, 0x40, L"ext-ms-win-shell-shlwapi-l1-1-0"};

extern long __stdcall ApiSetQueryApiSetPresence(PUNICODE_STRING ModuleName, unsigned char* PresenceFlag);

unsigned char __cdecl IsPathIsDirectoryWPresent(void)

{
  unsigned char uVar1;
  int iVar2;
  unsigned int in_ECX = 0;
  unsigned int uStack_8;
  
  if (ApiSetExists_ext_ms_win_shell_shlwapi_l1_1_0 == 1) {
    uVar1 = '\x01';
  }
  else {
    if (ApiSetExists_ext_ms_win_shell_shlwapi_l1_1_0 != 2) {
      uStack_8 = (unsigned int)in_ECX;
      iVar2 = ApiSetQueryApiSetPresence((PUNICODE_STRING)&ApiSetName_ext_ms_win_shell_shlwapi_l1_1_0,(unsigned char *)&uStack_8 + 3);
      if (-1 < iVar2) {
        ApiSetExists_ext_ms_win_shell_shlwapi_l1_1_0 = (uStack_8 == '\0') + 1;
        return (unsigned char)uStack_8;
      }
    }
    uVar1 = '\0';
  }
...
//    itoa(ConsoleCP, &szConsoleCP[1], 10);
    sprintf_s(szConsoleCP, SCE_STATUS_NEW_SERVICE, ".%d", ConsoleCP);
...
//    setlocale(LC_ALL, ".OCP");
	...
	if (!bIsAdmin) {
		ScepPrintCmdLineHelp(7);   // prints SECEDITP_OUT_OF_PERMS
		rCode = ERROR_ELEVATION_REQUIRED;             // ERROR_ELEVATION_REQUIRED (740)
		goto Done;
	}
    ...
    )
{
    ...
    switch (EngineType) {
    case 1:  dwErrorId = SECEDITP_GENERATE_HELP;  break; // SCE_ENGINE_GENERATE
    case 2:  dwErrorId = SECEDITP_VALIDATE_HELP;  break; // SCE_ENGINE_COMPILE
    case 3:  dwErrorId = SECEDITP_REGISTER_HELP;  break; // SCE_ENGINE_REGISTER
    case 5:  dwErrorId = SECEDITP_BROWSE_HELP;    break; // SCE_ENGINE_BROWSE
    case 6:  dwErrorId = SECEDITP_IMPORT_HELP;    break; // SCE_ENGINE_IMPORT
    case 7:  dwErrorId = SECEDITP_OUT_OF_PERMS;   break; // <-- admin failure
    case 0x12e: dwErrorId = SECEDITP_CONFIGURE_HELP; break; // SCE_ENGINE_SCP
    case 0x12f: dwErrorId = SECEDITP_ANALYZE_HELP;   break; // SCE_ENGINE_SAP
    case 0x141: dwErrorId = SECEDITP_ROLLBACK_HELP;  break; // SCE_ENGINE_RBK
    default:    dwErrorId = SECEDITP_SIMPLE_HELP;    break;
    }

    pszLineString = (PWSTR) LocalAlloc(LMEM_ZEROINIT, 3072 * sizeof(WCHAR));
    if (pszLineString) {
        LoadString(hMod, dwErrorId, pszLineString, 3072);
    ...
    )
{
    ...
    const wchar_t*       FirstThree = NULL;
    WCHAR       LineString[256];
	PSHORT      local_220 = NULL;
	PSHORT      local_214 = NULL;
	const wchar_t* local_10;
    ...
    wcsncpy_s(pUserFilename, 3, FirstThree, 4);
    FirstThree = L'\0';
    ...
    if (NeedCurrDirFlag){
        ...
        if  (pCurrentDir[LenCurrDir - 2] != L'\\')
            wcscat_s(pCurrentDir, (unsigned int)local_220 >> 1, L"\\");
    }
    ...
    if (NeedCurrDirFlag)
        wcscat_s(pToMerge, (unsigned int)local_214 >> 1, pCurrentDir);

    wcscat_s(pToMerge, (unsigned int)local_214 >> 1, pUserFilename);
    ...
    wcscpy_s(pAbsolutePathDirOnly, uVar10 >> 1, pAbsolutePath);
    pLastSlash = wcsrchr(pAbsolutePathDirOnly, L'\\');
    if (pLastSlash)
        *pLastSlash = L'\0';
	uVar3 = IsPathIsDirectoryWPresent();
    if ((uVar3 != '\0') && (PathIsDirectory(pAbsolutePathDirOnly) == FALSE)){
        LoadString( hMod,
