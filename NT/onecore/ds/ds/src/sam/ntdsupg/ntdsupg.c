...
#include <lmerr.h>
#include <limits.h>
#include <crtdbg.h>  // For _ASSERT or _ASSERTE
#include <winternl.h>
...
Cleanup:
    ...
    _ASSERT(WinError == ERROR_SUCCESS);

    return;
}

BOOL IsDomainControllerAlternative();
    ...
    LPVOID Context)
{
    ...
    // get OS version and product type
    ...
    if (VER_PLATFORM_WIN32_NT == osvi.dwPlatformId)
    {
        ...
        {
            // Alternative method to detect domain controller
            // Check if this is a domain controller via registry or other means
            bDomainController = IsDomainControllerAlternative();
        }
...
// Alternative function to detect domain controller
BOOL IsDomainControllerAlternative()
{
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, 
        ...
        if (RegQueryValueEx(hKey, L"ProductType", NULL, &dwType, 
                           (LPBYTE)buffer, &dwSize) == ERROR_SUCCESS) {
            RegCloseKey(hKey);
            return (_wcsicmp(buffer, L"LanmanNT") == 0);
        }
        RegCloseKey(hKey);
    }
    
    // Method 2: Use NetServerGetInfo
    // This requires linking with netapi32.lib
    return FALSE; // Default to not DC if we can't determine
}
    ...
    LPVOID Context)
{
    ...
    if (VER_PLATFORM_WIN32_NT == osvi.dwPlatformId)
    {
        ...
        {
            // Use the alternative method to detect domain controller
            bDomainController = IsDomainControllerAlternative();
        }
