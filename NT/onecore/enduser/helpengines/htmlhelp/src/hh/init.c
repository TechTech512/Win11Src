...
#if defined(_M_IX86)
extern int __cdecl atexit(_PVFV func);
#endif
...
{
  return wsprintf( (LPWSTR) pszBuf,
    (LPCWSTR)txtStringGuid,
...
BOOL GetRegisteredLocation( CLSID riid, LPTSTR pszPathname )
{
  ...
  wsprintf( (LPWSTR)szScratch, (LPCWSTR)txtInProc, szGuidStr );
  if( RegOpenKeyEx( HKEY_CLASSES_ROOT, (LPWSTR)szScratch, 0, KEY_READ, &hKey ) == ERROR_SUCCESS ) {
    DWORD dwSize = MAX_PATH;
    if( RegQueryValueEx( hKey, L"", 0, 0, szScratch, &dwSize ) == ERROR_SUCCESS ) {
      lstrcpy( pszPathname, (LPWSTR)szScratch );
      bReturn = TRUE;
    }
  }
    ...
    int iCmdShow)
{
    ...
    if( !GetRegisteredLocation( CLSID_HHCtrl, szHHCtrl ) )
      lstrcpy( szHHCtrl, (LPCWSTR)txtHHCtrl );

    hmodHHCtrl = (HMODULE) LoadLibrary( szHHCtrl );
    if( !hmodHHCtrl )
      hmodHHCtrl = (HMODULE) LoadLibrary( (LPCWSTR)txtHHCtrl );
